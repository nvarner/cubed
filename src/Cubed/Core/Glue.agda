...
open import Agda.Builtin.Cubical.Glue public
  using ( _≃_           -- ∀ {ℓ ℓ'} (A : Type ℓ) (B : Type ℓ') → Type (ℓ ⊔ ℓ')
  ...
  renaming ( isEquiv     to is-equiv         -- ∀ {ℓ ℓ'} {A : Type ℓ} {B : Type ℓ'} (f : A → B) → Type (ℓ ⊔ ℓ')

           ; equiv-proof to has-equiv-proof  -- ∀ (y : B) → isContr (fiber f y)

           ; equivFun    to equiv-fun        -- ∀ {ℓ ℓ'} {A : Type ℓ} {B : Type ℓ'} → A ≃ B → A → B

           ; equivProof  to equiv-proof      -- ∀ {ℓ ℓ'} (T : Type ℓ) (A : Type ℓ') (w : T ≃ A) (a : A) φ →
                                             -- Partial φ (fiber (equivFun w) a) → fiber (equivFun w) a

           ; prim^glue   to glue             -- ∀ {ℓ ℓ'} {A : Type ℓ} {φ : I} {T : Partial φ (Type ℓ')}
...
private module @0 _ where
  ...
  module GluePrims (A : Type ℓ) {φ : I} (Te : Partial φ (Σ[ T ∈ Type ℓ' ] T ≃ A)) where
    ...
    trans-e :
      ∀ (t : PartialP φ T)
      → Partial φ A
    trans-e t ϕ1 = equiv-fun (e ϕ1) (t ϕ1)
